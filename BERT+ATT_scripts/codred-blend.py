...
def process_example(h, t, doc1, doc2, tokenizer, max_len, redisd, no_additional_marker, mask_entity):
    doc1 = json.loads(redisd.get('codred-doc-open-' + doc1))
    doc2 = json.loads(redisd.get('codred-doc-open-' + doc2))
    v_h = None
...
def collate_fn(batch, args, relation2id, tokenizer, redisd, index_file=None):
    assert len(batch) == 1
    if batch[0][-1] == 'o':
        batch = batch[0]
        h, t = batch[0].split('#')
        ...
        for doc1, doc2, l in dps:
            tokens, token_ids, token_type_id, amask = get_passage_context(h, t, doc1, doc2, tokenizer, redisd, index_file, args.seq_len, args.expand, args.bridge_only, args.add_special)
...
def collate_fn_infer(batch, args, relation2id, tokenizer, redisd, index_file=None):
    assert len(batch) == 1
    batch = batch[0]
    h, t = batch[0].split('#')
    ...
    attention_mask = list()
    for doc1, doc2, l in dps:
        tokens, token_ids, token_type_id, amask = get_passage_context(h, t, doc1, doc2, tokenizer, redisd, index_file, args.seq_len, args.bridge_only, args.add_special)
        input_ids.append(token_ids)
        token_type_ids.append(token_type_id)
        attention_mask.append(amask)
    input_ids_t = torch.tensor(input_ids, dtype=torch.int64)
...
class Codred(torch.nn.Module):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, dplabel=None, rs=None):
        ...
        embedding, *other = self.bert(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, return_dict=False)
...
class CodredCallback(TrainerCallback):
    ...
    def on_argument(self, parser):
        ...
        parser.add_argument('--bridge_only', action='store_true')
        parser.add_argument('--add_special', action='store_true')
        ...
        parser.add_argument('--test_file', type=str, default='../data/rawdata/test_dataset_closed.json')
        parser.add_argument('--dsre_file', type=str, default='../data/dsre_train_examples.json')
        parser.add_argument('--index_file', type=str, default='../data/retrieval_index/mdr_finetune_inference.json')
    ...
    def load_data(self):
        self.index_file = json.load(open(self.args.index_file))
    ...
    def collate_fn(self):
        return partial(collate_fn, args=self.args, relation2id=self.relation2id, tokenizer=self.tokenizer, redisd=self.redisd, index_file=self.index_file), partial(collate_fn_infer, args=self.args, relation2id=self.relation2id, tokenizer=self.tokenizer, redisd=self.redisd, index_file=self.index_file), partial(collate_fn_infer, args=self.args, relation2id=self.relation2id, tokenizer=self.tokenizer, redisd=self.redisd, index_file = self.index_file)
    ...
    def on_dev_epoch_end(self, epoch):
        ...
        for p, score, h, t, rs in self._prediction:
            ...
            results.append([h, rs, t, self.relations[p], score[p]])
            for r in rs:
                if r != 'n/a':
                    facts[(h, t, r)] = 1
    ...
    def on_test_epoch_end(self, epoch):
        ...
        with self.trainer.once():
            #self.dev_logger.log(dev_mean_prec=stat['mean_prec'], dev_f1=stat['f1'], dev_auc=stat['auc'])
            #json.dump(stat, open(f'output/test-stat-{epoch}.json', 'w'))
            json.dump(pred_result, open(f'output/test-results-{epoch}.json', 'w'))
        return 0.0
